...
def load_data(dataset, batch_size, context_length, device='cpu', num_worker = 1):
    ...
    with ThreadPoolExecutor(max_workers=num_worker) as executor:
        sequences = torch.stack(list(executor.map(lambda start: torch.LongTensor(dataset[start:start+context_length].copy()).to(device), start_indices)))
        labels = torch.stack(list(executor.map(lambda start: torch.LongTensor(dataset[start+1:start+context_length+1].copy()).to(device), start_indices)))
    
    return sequences, labels

class Dataset:
    def __init__(self, dataset_path, dtype=np.uint16):
        self.data = np.memmap(dataset_path, dtype=dtype, mode='r')
